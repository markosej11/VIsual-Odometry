...
def main(argv):
    ...
    positions = np.zeros((0, 4, 4), dtype=np.float64)
    trajectory = []
    ...
    for i in range(len(files) - 1):
        ...
        # Inbuilt openCV function to getting essential matrix
        cv_ess_mat, _ = cv2.findEssentialMat(kp1, kp2, K, method=cv2.RANSAC, prob=0.999,
                                             threshold=0.99)

        # Inbuilt OpenCV functions for best pose
        _, cv_rot_mat, cv_trans_mat, mask = cv2.recoverPose(cv_ess_mat, kp1, kp2, K)
        cv_base_pose_new = np.array([[0, 0, 0, 1]]).T
        cv_x_ = np.column_stack((cv_rot_mat, cv_trans_mat))
        cv_y_ = np.array([0, 0, 0, 1])
        cv_base_pose = cv_base_pose @ np.vstack((cv_x_, cv_y_))
        points = cv_base_pose @ cv_base_pose_new
        trajectory.append([points[0][0], -points[2][0]])
